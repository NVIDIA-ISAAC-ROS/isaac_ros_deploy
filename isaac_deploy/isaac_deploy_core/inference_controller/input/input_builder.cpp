...
InputBuilder::Config::create_from_model_config(const ModelConfig & model_config)
{
  ...
  for (const auto & term_yaml : model_config.inputs) {
    ...
    // Apply feedback flow: for each output_name -> [input_names],
  ...
  for (const auto &[output_name, input_names] : model_config.feedback_flow) {
    for (const auto & input_name : input_names) {
      ...
      if (it == config.terms.end()) {
        return tl::unexpected(
            make_error(
              Error::Code::kInvalidArgument,
              "feedback_flow references unknown input '" + input_name + "'"));
      }
      ...
      if (!it->source.empty() && it->source != output_name) {
        return tl::unexpected(
            make_error(
              Error::Code::kInvalidArgument,
              "feedback_flow: input '" + input_name +
    ...
    // Apply source defaults now that feedback flow is resolved.
