...
public:
  ...
  struct Config
  {
    ...
    std::vector<OutputTermConfig> terms;
      ...
      /// Defaults to 1/30 s (30 Hz) if not declared.
    double policy_dt_seconds = 1.0 / 30.0;
  ...
  std::vector<std::string> get_element_names(const std::string & output_name) const;
    ...
    /// Defaults to 1/30 s (30 Hz) if semantic.scene.dt is not declared.
...
private:
  ...
  double policy_dt_seconds_ = 1.0 / 30.0;
};
