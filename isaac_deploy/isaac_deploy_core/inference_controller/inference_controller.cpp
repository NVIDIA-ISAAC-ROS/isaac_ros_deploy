...
: input_builder_(std::move(input_builder)),
  ...
  feedback_initial_values_(std::move(feedback_initial_values)) {}
...
expected<InferenceController> InferenceController::create(InferenceControllerConfig config)
{
  ...
  return InferenceController(
      ...
      std::move(*output_builder_result), std::move(config.feedback_initial_values));
}

expected<void> InferenceController::warmup()
{
  ...
  std::vector<NamedTensor> & outputs)
{
  ...
  for (const auto & source : all_sources) {
    ...
    if (feedback_it != feedback_shape_map.end()) {
      torch::Tensor tensor = torch::zeros(feedback_it->second);
      const auto initial_it = feedback_initial_values_.find(source);
      if (initial_it != feedback_initial_values_.end()) {
        if (initial_it->second.sizes().vec() != feedback_it->second) {
          return tl::unexpected(
              make_error(
                Error::Code::kInvalidArgument,
                "Initial feedback value for '" + source +
                "' has shape that does not match the configured feedback input"));
        }
        tensor = initial_it->second.clone();
      }
