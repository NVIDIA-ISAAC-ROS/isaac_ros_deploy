...
expected<std::unordered_map<std::string, std::vector<std::string>>>
parse_required_non_empty_connection_map(
  ...
  const std::string & section_name)
{
  if (!node.IsDefined()) {
    return tl::unexpected(
          make_error(
            Error::Code::kInvalidArgument,
            "pipeline." + section_name + " is required for multi-model configs"));
  }
  auto result = parse_connection_map(node, section_name);
  if (!result) {
  ...
  if (result->empty()) {
    return tl::unexpected(
          make_error(
            Error::Code::kInvalidArgument,
            "pipeline." + section_name + " must be a non-empty map for multi-model configs"));
  }
  return result;
}
...
expected<GraphConfig> parse_graph_config(const YAML::Node & root)
{
  ...
  GraphConfig graph;
  std::unordered_set<std::string> consumed_feedback_flow_keys;

    // Parse pipeline-level LEAPP connectivity before per-model feedback extraction.
  if (root["pipeline"].IsDefined()) {
    ...
    auto ff_result = parse_connection_map(pipeline["feedback_flow"], "feedback_flow");
    if (!ff_result) {
      return tl::unexpected(ff_result.error());
    }
    graph.feedback_flow = std::move(*ff_result);

    if (pipeline["initial_values"].IsDefined()) {
      graph.initial_values_path = pipeline["initial_values"].as<std::string>();
    }
  }
  ...
  for (const auto & model_entry : models) {
    ...
    if (model["parameters"].IsDefined()) {
      ...
      // Per-model feedback_flow entries are exposed internally as output_name -> input names.
    for (const auto & [full_key, targets] : graph.feedback_flow) {
      ...
      if (key_model == model_name) {
        consumed_feedback_flow_keys.insert(full_key);
        ...
        if (!output_exists) {
          return tl::unexpected(
              make_error(
                Error::Code::kInvalidArgument,
                "feedback_flow key '" + full_key +
                "' refers to unknown output '" + key_name +
                "' in model '" + model_name + "'"));
        }
        std::vector<std::string> input_names;
        for (const auto & inp : targets) {
          input_names.push_back(strip_model_prefix(inp));
        }
        config.feedback_flow[key_name] = std::move(input_names);
      }
    }
  ...
  for (const auto & [full_key, targets] : graph.feedback_flow) {
    (void)targets;
    if (!consumed_feedback_flow_keys.contains(full_key)) {
      return tl::unexpected(
          make_error(
            Error::Code::kInvalidArgument,
            "feedback_flow key '" + full_key +
            "' does not reference a declared model output"));
    }
  }
  ...
  const YAML::Node & root)
{
  ...
  auto inputs_result = parse_required_non_empty_connection_map(
  ...
  auto outputs_result = parse_required_non_empty_connection_map(
  ...
  config.outputs = merged_outputs;
    ...
    // Feedback flow is handled by InferenceRunnerNodes, not InputBuilder.
    // Leave feedback_flow empty in the merged config.
